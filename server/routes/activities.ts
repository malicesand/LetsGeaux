...
activityRouter.get('/google-place-autocomplete', async (req: any, res: any) => {
  ...
  try {
    ...
    if (response.data.status !== 'OK') {
      return res
        .status(400)
        .json({ error: response.data.error_message || 'Google API error' });
    }
    ...
    res
      .status(500)
      .json({ error: 'An error  fetching autocomplete suggestions.' });
  }
});
...
activityRouter.get('/autocomplete-descriptions', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Failed to fetch autocomplete descriptions' });
  }
});
...
activityRouter.patch('/:id', async (req: any, res: any) => {
  try {
    ...
    const activity = await prisma.activity.findUnique({
      where: { id: Number(id) },
      include: {
        itinerary: { include: { party: { include: { userParty: true } } } },
      }
    });
    ...
    const isInParty = activity.itinerary.party?.userParty.some(
      up => up.userId === userId
    );
    ...
    if (!isCreator && !isInParty) {
      return res
        .status(403)
        .json({ error: 'Not authorized to update this activity.' });
    }
